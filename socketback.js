...
//to enable cors
app.use(cors({credentials: true, origin: 'http://localhost:4200,https://app.vitutors.net'}));
var fs = require('fs');
var certDir = `/etc/letsencrypt/live`;
var domain = `vitutors.net-0001`;
var options = {
...
app.use(function (req, res, next) {
    ...
    res.setHeader('Access-Control-Allow-Origin', 'http://localhost:4200,https://app.vitutors.net');
...
var https = require('https').createServer(options, app).listen(8000);
                ...
                if (!insertError) {
                    db.query("SELECT * from user_profiles INNER JOIN users ON users.id = user_profiles.user_id where user_profiles.user_id = '" + receiver_id + "'", async function (err, senderrow) {
                        ...
                        if(senderrow[0].role_id == 3){
                            message_board_url = 'https://app.vitutors.net/tutor/message-board' ;
                        }else{
                            message_board_url = 'https://app.vitutors.net/student/message-board' ;
                        }
                    ...
                    db.query("SELECT * from user_profiles INNER JOIN users ON users.id = user_profiles.user_id where user_profiles.user_id = " + receiver_id , async function (err, senderrow) {
                        if(!err){
                            ...
                            if(senderrow[0].role_id == 1){
                                message_board_url = 'https://admin.vitutors.net/admin/message-board';
