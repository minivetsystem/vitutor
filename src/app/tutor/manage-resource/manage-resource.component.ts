...
export class ManageResourceComponent implements OnInit {
  ...
  onSubmit(form_type){
    ...
    let url = this.createFolderUrl;
    let value = this.addResourceFolderForm.value;
    if ((form_type == 'Update') && (this.folderId != null)) {
      url = this.updateFolder;
      value = Object.assign({}, this.addResourceFolderForm.value, { 'folder_id' : this.folderId});
    }
  ...
  cdkDragMoved = (event: CdkDragMove<any>) => {
    ...
    if (elemBelow) {
    droppableRow = elemBelow.closest('.categories-item');
    if (!droppableRow) { // dropeed in between the projects
    droppableRow = elemBelow.closest('.new-row');
    }
    }
    ...
    allRows[x].className = ".categories-item";
    }
    }
  ...
  doubleClickFolder(item){
    this.router.navigate(['/tutor/resource-folder/' , item.id])
  }
}
